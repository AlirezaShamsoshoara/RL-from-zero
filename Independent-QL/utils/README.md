# Independent-QL utils - README asset generators

Helpers that produce the demo assets in `Independent-QL/assets/`:

- **`make_gif.py` / `make_gif.sh`** - roll out the greedy joint policy on LineWorld
  and render each step (via `LineWorldEnv.render`) into an animated GIF.
...
`independent_ql` package on `sys.path` themselves (the folder is hyphenated, so
it is not importable as `Independent-QL`).

## Requirements
Project deps plus `imageio` + `pillow` (GIF) and `matplotlib` (charts and the
LineWorld renderer). These are in `pyproject.toml`; the `rlhero` conda env has them.

## GIF generator
```bash
PYTHON=~/.conda/envs/rlhero/bin/python ./Independent-QL/utils/make_gif.sh --episodes 3
```
...
| `--fps` | `4` | Frames per second (LineWorld is slow-moving, so a low fps reads well). |
...
## Notes
- **No display needed:** LineWorld renders with matplotlib (Agg backend), so the
