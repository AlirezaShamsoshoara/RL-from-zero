...
class LineWorldEnv:
    ...
    def reset(self, seed: Optional[int] = None) -> List[int]:
        ...
        # Per-agent "done" flags: once an agent reaches its goal it freezes there
        # (it stops moving and stops accruing reward), so a finished agent cannot
        # farm the goal reward while waiting for its teammate.
    ...
    def step(self, actions: Sequence[int]) -> StepResult:
        ...
        # Active agents pay the step penalty; finished agents get nothing further.
    ...
    def render(self, title: Optional[str] = None):
        ...
        keeps overlapping positions readable. Used by the demo GIF tool.
        """
        import matplotlib
