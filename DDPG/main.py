...
from DDPG.ddpg.agent import DDPGAgent, DDPGStats
from DDPG.ddpg.replay_buffer import ReplayBuffer
from DDPG.ddpg.logging_utils import setup_logger


def _stats_to_dict(stats: DDPGStats) -> Dict[str, float]:
    return asdict(stats)
...
def train(config: str = "DDPG/configs/pendulum.yaml", wandb_key: str = ""):
    ...
    pbar = tqdm(range(1, cfg.total_steps + 1), desc="DDPG Steps")
    for step in pbar:
